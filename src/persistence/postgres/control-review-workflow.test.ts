...
async function tempStack() {
  ...
  return {
    ...
    records: createPostgresControlRecordRepository(db),
...
describe("reviewStatus defaults", () => {
  it("defaults reviewStatus to not_reviewed for new persisted rows", async () => {
    const { projects, service } = await tempStack();
  ...
  it("migrated existing-style rows read as not_reviewed via schema default", async () => {
    const { db, projects, records } = await tempStack();
    const project = await projects.create({
      name: "Legacy",
      frameworkId: NIST_MODERATE_FRAMEWORK_ID,
    });
    // Insert omitting review_status so PostgreSQL applies the column default.
...
describe("review workflow transitions", () => {
  async function advance(
    service: ReturnType<typeof createPostgresControlRecordService>,
    projectId: string,
    controlId: string,
  ...
  it("applies every legal transition with correct activity payload", async () => {
    const { projects, service, activities } = await tempStack();
    const project = await projects.create({
      name: "Happy path",
      frameworkId: NIST_MODERATE_FRAMEWORK_ID,
    });
  ...
  it("rolls back ControlRecord and activity if append fails mid-transition", async () => {
    const { db, projects, activities } = await tempStack();
    const project = await projects.create({
      name: "Tx fail",
      frameworkId: NIST_MODERATE_FRAMEWORK_ID,
    });
    ...
    await assert.rejects(
      () =>
        db.transaction(async (tx) => {
          ...
          await tx.insert(controlActivities).values({
            id: "act-review-tx",
    ...
    assert.equal(
      (await activities.listByControlRecordId("cr-review-tx")).length,
      0,
    );
  });
