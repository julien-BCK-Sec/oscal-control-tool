...
function tempRepo() {
  const dir = mkdtempSync(join(tmpdir(), "oscal-db-"));
  dirs.push(dir);
...
describe("ProjectRepository", () => {
  ...
  it("reports corrupt project_json without overwriting", async () => {
    ...
    sqlite
      .prepare("UPDATE projects SET project_json = ? WHERE id = ?")
      .run("{not-json", created.id);
    sqlite.close();

    openDbAt(dbPath);
    const repo2 = createSqliteProjectRepository(openDbAt(dbPath));
    const loaded = await repo2.load(created.id);
...
describe("snapshots and versions", () => {
  ...
  it("throttles automatic snapshots when respectThrottle is true", async () => {
    ...
    const sqlite = new Database(dbPath);
    sqlite
      .prepare("UPDATE project_snapshots SET created_at = ? WHERE id = ?")
      .run(new Date(Date.now() - 6 * 60 * 1000).toISOString(), first!.id);
    sqlite.close();

    const repo2 = createSqliteProjectRepository(openDbAt(dbPath));
    const allowed = await repo2.createAutomaticSnapshot(created.id);
    assert.ok(allowed);
  });
